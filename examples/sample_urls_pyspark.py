...
from pyspark.sql.functions import expr, col, rand, row_number
...
def sample_x_random_rows_per_group(df, group_col, x):
    window_spec = Window().partitionBy(group_col)
    sampled_df = df.withColumn("rand", rand()) \
        .withColumn("row_number", row_number().over(window_spec.orderBy("rand"))) \
        .filter(col("row_number") <= x) \
        .drop("rand", "row_number")
    return sampled_df
